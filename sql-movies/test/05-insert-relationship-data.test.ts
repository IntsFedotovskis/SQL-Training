...
const insertMovieGenres = (
  ...
  return `insert into ${MOVIE_GENRES} (movie_id, genre_id) values` + filtred.map(genres => `(${movieId}, ${genres.id})`).join(",")
};
...
const insertMovieActors = (
  ...
  return `insert into ${MOVIE_ACTORS} (movie_id, actor_id) values` + filtred.map(actors => `(${movieId}, ${actors.id})`).join(",")
};
...
const insertMovieDirectors = (
  ...
  return `insert into ${MOVIE_DIRECTORS} (movie_id, director_id) values` + filtred.map(directors => `(${movieId}, ${directors.id})`).join(",")
};
...
const insertMovieKeywords = (
  ...
  return `insert into ${MOVIE_KEYWORDS} (movie_id, keyword_id) values` + filtred.map(keywords => `(${movieId}, ${keywords.id})`).join(",")
};
...
const insertMovieProductionCompanies = (
  ...
  return `insert into ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) values` + filtred.map(productionCompanies => `(${movieId}, ${productionCompanies.id})`).join(",")
};
