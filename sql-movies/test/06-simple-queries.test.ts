...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `
      SELECT count(homepage) as count
      FROM ${MOVIES} 
  ...
  it(
    ...
    async done => {
      const query = `
      SELECT COUNT(id) as count,substr(release_date,1,4) as year
      FROM ${MOVIES} 
      GROUP BY year
      ORDER BY year
      DESC LIMIT 8
  ...
  it(
    ...
    async done => {
      const query = `
      SELECT COUNT(rating) as count,substr(time_created,6,2) as month
      FROM ${MOVIE_RATINGS} 
      GROUP BY month
      ORDER BY count
      DESC LIMIT 12
